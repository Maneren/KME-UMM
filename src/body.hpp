...
public:
  ...
  virtual void initialize() override {
    ...
    _mesh = create_mesh();
    std::println("triangles: {}", _mesh.triangleCount);
    _model.Load(_mesh);
...
protected:
  ...
  void body_apply_force(
    ...
    const raylib::Vector3 torque = offset.CrossProduct(force);
    const float moment = moment_of_inertia(torque);
    std::println(
        "force: {}, offset: {} -> torque: {} -> moment: {} -> acceleration: {}",
        force,
        offset,
        torque,
        moment,
...
private:
  void update_position(const float delta) {
    ...
    _acceleration = raylib::Vector3{0.0f, 0.0f, 0.0f};
  }
  ...
  void update_orientation(const float delta) {
    ...
    const auto axis =
        _angular_velocity * (std::sin(angle * delta / 2.0f) / angle);

    const raylib::Quaternion rotation{
        axis.x, axis.y, axis.z, std::cos(angle * delta / 2.0f)
    };

    std::println("orientation: {}, rotation: {}", _orientation, rotation);
    _orientation = (_orientation * rotation).Normalize();
    _model.transform = _orientation.ToMatrix();
    std::println("orientation after: {}", _orientation);
  }
};
